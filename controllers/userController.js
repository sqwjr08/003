...
exports.list = async (req, res) => {
  ...
  let limit = Number(req.query.limit || 15); //注意验证 是否为数字 //限制条数
...
// 用户注册
// 处理用户注册请求
// 普通用户注册时role默认为'member'，class默认为'none'
// 管理员可以指定role和class
exports.add = async (req, res) => {
  ...
  if (!authController.isAdmin(req)) {
    ...
    userData.class = "none"; // 新增：普通用户注册时class默认为none
  }
...
exports.update = async (req, res) => {
  ...
  if (authController.isAdmin(req)) {
    ...
    findUser.class = userData.class; // 新增：管理员可以更改用户的class
  }
  let result = null;
  if (userData.password) {
